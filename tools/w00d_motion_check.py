...
1. **原地慢转**:``wz=--turn-wz``(默认 0.3 rad/s)转 ``--turn-s`` 秒,看里程里转了多少、往哪边转
   (HAL 约定正值逆时针;反了就给代理加 ``--invert-yaw``),折算 ``radps_per_unit``。
2. **前进**:``vx=--fwd-vx``(默认 0.3 m/s)走 ``--fwd-s`` 秒(默认 2 s),折算 ``mps_per_unit``。
3. **停止用时**:第 2 步末尾发 ``stop``,量到里程速度归零用了多久。
4. **ttl 到期自停**:发一条 ``ttl=500 ms`` 的速度后不再续,量多久停。
...
``--mps-per-unit`` / ``--radps-per-unit`` / ``--invert-yaw``。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--mps-per-unit", type=float, default=0.4, help="当前配置值(待测)")
    p.add_argument("--radps-per-unit", type=float, default=1.0, help="当前配置值(待测)")
    p.add_argument("--turn-wz", type=float, default=0.3)
    p.add_argument("--turn-s", type=float, default=3.0)
    p.add_argument("--fwd-vx", type=float, default=0.3)
...
async def _drive(hal: Any, vx: float, wz: float, seconds: float) -> None:
    ...
    while time.monotonic() < t_end:
        seq += 1
        got = await hal.set_velocity(VelocityCommand(seq=seq, ttl_ms=300, frame="base",
                                                     vx=vx, vy=0.0, wz=wz))
        if got.rejected:
...
async def check(args: argparse.Namespace) -> dict[str, Any]:
    ...
    from d1max_contract.hal import MotionStatus, VelocityCommand
    ...
    # 死区交给第 5 步去量,HAL 这里不挡(deadband_mps=0)。
    hal = D1MaxHal(host, port, backend=b, mps_per_unit=args.mps_per_unit,
                   radps_per_unit=args.radps_per_unit, deadband_mps=0.0)
    out: dict[str, Any] = {"at": time.strftime("%Y-%m-%dT%H:%M:%S"), "sidecar": f"{host}:{port}",
                           "config": {"mps_per_unit": args.mps_per_unit,
                                      "radps_per_unit": args.radps_per_unit},
                           "steps": {}}
    await hal.connect()
    try:
        ...
        await _drive(hal, 0.0, args.turn_wz, args.turn_s)
        await hal.stop()
        settle = await _settle(hal)
        o1 = await hal.odometry()
        dyaw = _wrap(o1.yaw - o0.yaw)
        rate = dyaw / args.turn_s
        steps["turn"] = {"cmd_wz": args.turn_wz, "seconds": args.turn_s,
                         "yaw_delta_rad": round(dyaw, 4), "rate_radps": round(rate, 4),
                         "direction_ok": dyaw * args.turn_wz > 0, "settle_s": settle}
        if settle is None:
            raise RuntimeError("转完停不稳")

        # 2、3. 前进 + 停止用时
        o0 = await hal.odometry()
        await _drive(hal, args.fwd_vx, 0.0, args.fwd_s)
        t_stop = time.monotonic()
        await hal.stop()
        settle = await _settle(hal)
        o1 = await hal.odometry()
        dist = math.hypot(o1.x - o0.x, o1.y - o0.y)
        steps["forward"] = {"cmd_vx": args.fwd_vx, "seconds": args.fwd_s,
        ...
        # 4. ttl 到期自停
        t0 = time.monotonic()
        got = await hal.set_velocity(VelocityCommand(seq=1, ttl_ms=500, frame="base",
                                                     vx=args.fwd_vx, vy=0.0, wz=0.0))
        if got.rejected:
            raise RuntimeError(f"速度被拒:{got.reason}")
        ...
        fwd_frac = args.fwd_vx / args.mps_per_unit
        turn_frac = args.turn_wz / args.radps_per_unit
        out["suggest"] = {
            "mps_per_unit": round(steps["forward"]["speed_mps"] / fwd_frac, 3),
            "radps_per_unit": round(abs(rate) / turn_frac, 3),
...
def _check_limits(p: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    if not 0 < abs(args.fwd_vx) <= MAX_VX:
        p.error(f"--fwd-vx 要在 (0, {MAX_VX}]")
    if not 0 < abs(args.turn_wz) <= MAX_WZ:
        p.error(f"--turn-wz 要在 (0, {MAX_WZ}]")
