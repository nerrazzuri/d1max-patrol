...
def apply_sync(plan: SyncPlan, *, now_ms: int, robot_sn: str = "",
    ...
    **空计划也写进度 —— 但这条承诺是给手动同步那条路的。** 人点一下"同步",
    "同步过了,没有新东西"是一次成功的同步;不更新时间的那一边,他会看到
    "上次同步"停在很久以前,以为盘死了。**唯一的常驻调用方
    ``app/server.py`` 的 ``_autosync_once`` 刻意不走这一条**:空计划它直接
    跳过,不调这个函数。那条循环每分钟醒一次,照写就是每分钟一次 fsync,
    一块机械镜像盘从此永远不休眠。于是"这条循环还活着"的证据不是
    ``last_sync_ms``(自动同步不推它),**是 ``behind`` 不涨** —— 归档在增
    而落后数不涨,就说明有人在拷。
