...
#: 阶段 0 的地址探测表。
#:
#: 厂商两份文档给的导航地址**对不上**,而且两份都明确写着 D1 Max:
#:
...
#:
#: 无从判断哪份对(可能是同一台 Orin NX 的两块网卡,也可能是文档版本差),
#: 所以**两条都探**,让真机来回答。谁通用谁,答案写回 docs/真机待验证清单.md。
#:
#: SDK 那两条本卷没有实现,但**可达性本身就是要带回去的数据**:
#: 有线时 SDK 在 RK3588 的 192.168.168.168,走 WiFi AP 时在 192.168.234.1,
#: 端口都是 8081(《开发指南》2.7 的 example_data 两种连法都用 8081)。
#:
#: 8081 不是笔误,别"顺手改成 8082"。sdk_client_api_zh.md:136 写的是
#: **WebSocket 走 8081,UDP 走 8082**。这里探的是 TCP,而 TCP 连一个 UDP
#: 端口必然失败 —— 探 8082 只会稳定产出一条假阴性。要验 8082,得跑厂商
#: 自己的 example/data(见 docs/真机联调手册.md 的 S 阶段)。
