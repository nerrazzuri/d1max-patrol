...
#: 阶段 0 的地址探测表。
#:
#: **四条候选现场全部实测可达**(2026-09-01,见
#: ``runs/conformance/20260901T041557Z/conformance-report.md``)。所以这张表
#: 不再是"猜哪条对"的问卷,而是一次链路体检:哪条不通就说明网没配对。
#:
#: 导航两条都通 —— 厂商两份文档并不矛盾,是同一台 Orin NX 的两块网卡:
#:
#: - refs/nav-api/自主导航_WEBSOCKET_API.md:9 —— ``192.168.144.100:10010 #D1 Max``
#: - refs/hardware/《SDK 开发指南 V0.1.0》5.2 节的 control 例子 ——
#:   ``./control 192.168.168.168 8081 192.168.168.100 10010``,导航在 168.100
#:
#: 现场实际用的是 ``ws://192.168.168.100:10010`` —— 与 SDK 同网段,少配一条路由。
#:
#: **8081 那两条:"通"不等于"SDK 可用",别这么读。** 现场实测(清单 #34):
#: SDK 控制走 **8082/UDP**,连 8081 恒 ``ShakeHand failed``。8081 上确实有
#: 东西在听(所以 TCP 探测显示"通"),但它不是控制端点。
#:
#: 这里仍然只探 8081,理由不变:TCP 探测只验得了 TCP 端口,拿 TCP 去连
#: 8082 这个 UDP 口必然失败,只会稳定产出一条假阴性。所以这两条的含义
#: 降级为"SDK 主机网段可达";SDK 到底能不能用,由 ``motion/`` 下的 C++
#: 程序回答(见 docs/真机联调手册.md 的 S 阶段)。
DEFAULT_PROBES: tuple[tuple[str, str, int], ...] = (
    ("nav-144", "192.168.144.100", 10010),
    ("nav-168", "192.168.168.100", 10010),
    ("sdk-wired", "192.168.168.168", 8081),
    ("sdk-wifi", "192.168.234.1", 8081),
)
