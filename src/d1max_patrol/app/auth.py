...
#: 报名字 / 看现在记着谁的那条接口(挂账 90)。
#:
#: **这个字面量在这个仓库里只许出现一次。** 它以前在 ``server.py`` 里写了两
#: 遍(GET 一遍 PUT 一遍),手机那头 ``net/patrol_client.dart`` 里又写了一遍
#: —— 三份互相自洽的手抄件,改一处另外两处不会红。狗那侧改了路径,两侧的测试
#: **各自全绿**,而现场表现是「换人这件事到不了狗,留痕永远建不起来」。
#:
#: 手机那头对应的是 ``patrol_client.dart`` 的 ``operatorPath``。**两个常量还是
#: 两份手抄件**,只是从三份收成两份;真正把这条缝钉死的是
#: ``tests/app/test_operator.py::test_手机那头的路径跟狗这头是同一条``——
#: 它读 Dart 源码里那个常量,跟这里比。
#:
#: **它不在 ``OPEN_PATHS`` 里**:报名字要 token(``PUT``),看也要。
OPERATOR_PATH = "/api/operator"
...
class Throttle:
    ...
    **淘汰分两档,第一档优先,第二档是兜底** —— 完整规矩在 ``_sweep`` 里。
    正常情况下只扔没锁着的(``until <= now``),按最后一次失败从老到新;第一
    档扔完(不管扔了 0 条还是扔了一大半)上限仍被突破时,才动锁着的,挑**最
    快要解锁**的那些。
