...
class _StateHub:
    ...
    **为什么要备快照,而不是请求来了现问后端。** ``nav_status()`` 这类方法在
    ...
    async def _lease_gone(self, now_ms: int) -> None:
        ...
        **告警的文案按 ``engine.running`` 分两套写,这不是措辞讲究。**
        ...
        判"在不在跑"用 ``engine.running`` 这个引擎自己的属性,**不在外壳里
        比一遍状态表**(理由跟 ``yielding`` / ``loc_lost`` 那两条一样:哪些
        状态算"在跑"是引擎的词汇,外壳复刻一份迟早跟正主不一样)。

        **``running`` 为真不等于"腿这一刻在动"**:``PAUSED`` / ``SUSPENDED``
        也在 ``running`` 里头,那两档狗确实停着。所以那一支的正文**不许**写
        成"狗正在动",只写得出、也只需要写"这趟任务还活着、引擎没跟着停,
        狗停不停要看它在哪一档" —— 并把 ``snapshot.state`` 摆在正文里让人
        自己看。宁可让人多警惕一次,也不能让人照着一句"已停在原地"走过去。
        """
        try:
        ...
        self._ctx.alerts.raise_alert(
            ...
            title=("遥控租约到期,但这趟任务没有跟着停" if 跑着
                   else "遥控租约到期,狗已停在原地"),
            detail=(f"TTL 到了没人续租,遥控那一档已经断开(发过零速、手松了)。"
                    f"**但这趟巡检任务还活着,引擎一个字都没停** —— 它现在在 "
                    f"{engine.snapshot.state.value} 上,该走点位的时候会自己"
                    f"接着走、自己拍照。**不许默认它是静止的,别就这么走"
                    f"过去。** 要让它真停下来,回来重新取一次控制权,再按暂停"
                    f"或者中止;不管它的话它会自己把这趟跑完(§5.8)。"
                    if 跑着 else
                    "TTL 到了没人续租。已经停下,**没有自动续跑** —— "
                    "要接着干,回来重新取一次控制权(§5.8)。"),
            now_ms=now_ms)
