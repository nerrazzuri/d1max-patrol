...
默认只起导航那一台(CLI 够用了)。要彩排整个 app,得把 app 会去连的三条链路
都起上:``--full`` 就是干这个的,端口直接取 app 的默认值,起完 ``d1max-app``
不用带任何参数就能连上。
"""
...
from .map_server import SimMapServer
from .nav_server import SimNavServer

#: ``--full`` 用的端口。跟 ``d1max-app`` 的默认值对齐 —— 对不齐就得两边都带
#: 参数,而彩排时每多一个参数就多一处"现场跟在家不一样"。
FULL_NAV_PORT = 10010
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--map-port", type=int, default=0,
                   help="同时起一台仿真建图桥,0 为不起")
    p.add_argument("--reveal", type=float, default=0.6,
                   help="建图桥先露出多大比例的图(0~1,默认 %(default)s)")
    p.add_argument("--full", action="store_true",
                   help=f"三台一起起并预置地图。旁路 {FULL_AGENT_PORT} / 建图桥 "
                        f"{FULL_MAP_PORT} 直接是 app 的默认值;导航 "
                        f"{FULL_NAV_PORT} 那条要给 app 带 --nav-url,"
                        "因为它的默认值是机器狗的 IP")
...
async def _serve(args: argparse.Namespace) -> None:
    ...
    extra: list[SimAgentServer | SimMapServer] = []
    try:
        if args.agent_port:
            ...
            print(f"旁路进程: {args.host}:{agent.port}  (motion={agent.motion.value},"
                  f" 站起来要自己发命令,跟真机一样)")
        if args.map_port:
            maps = SimMapServer(host=args.host, port=args.map_port)
            # 先露出一部分:全是未知的话页面上就是一片灰,看不出桥通没通。
            maps.reveal(args.reveal)
            await maps.start()
            extra.append(maps)
            print(f"建图桥: {args.host}:{maps.port}  (已露出 {args.reveal:.0%})")
        if extra:
            # 导航地址得显式给:app 的默认值是机器狗的 IP,不是本机。
            print()
            print("彩排 app,另开一个终端:")
            print(f"  d1max-app --nav-url {server.url}"
                  " --ffmpeg scripts/fake-camera.py")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.full:
        ...
        args.map_port = args.map_port or FULL_MAP_PORT
        args.seed = True
    logging.basicConfig(
