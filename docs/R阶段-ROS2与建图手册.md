...
> **⚠ 2026-09-02 已被现场实践部分取代。** 这一趟真做完了，结果见
> [`建图定位与巡检管线.md`](建图定位与巡检管线.md)，侦察项的答案回填在
> [`真机待验证清单.md`](真机待验证清单.md) 的「现场结果回填」一节。
> 下面这份仍是有用的**背景与推理**，但**别再照着它的命令跑**：
> R1.2 改用 `ZENOH_ROUTER_CONFIG_URI` 指向 `config/zenoh_router.json5`（不要 sudo 改 `/opt/ros`）；
> 链路走的是 WiFi 不是网线；建图用速腾 `/front_lidar` 拍扁，不用厂商的 `/laser_scan`。
