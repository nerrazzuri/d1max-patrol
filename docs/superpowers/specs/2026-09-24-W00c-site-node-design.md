...
- 决策 5（「服务器不许有移动能力」改为「可发任务级指令、不可遥控」）仍在「待人拍板」表里。c1 派 `goto` 本身就是任务级指令，**严格说也落在决策 5 上**；总设计 §7 写了「决策 5 由此有答案」，但工单表没勾。拍这份设计时请一并确认决策 5。
