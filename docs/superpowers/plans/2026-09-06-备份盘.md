...
- **禁止删除或跳过任何已有测试。** 全仓收集数只许涨。基线 **1782**，本卷做完应为 **1888**。
...
收集数核对：
`python -m pytest --collect-only -q 2>&1 | awk -F': ' '/^tests.*: [0-9]+$/{s+=$2} END{print s}'`
Expected: **1851**（终值 **1888**：任务 8 的评审补了 3 条，终审修复轮又补了 30 条，残留修复轮又补了 4 条，见文末）
...
**4. 测试数核对：** 9 + 8 + 7 + 10 + 9 + 6 + 11 + 12 = **72** 条新测试函数。基线 1782，做完 **1888**。

终审修复轮（4 条 Critical + 8 条 Important + 4 条 Minor）又补了 30 条收集项，其中最大的一块是 C4 新增的自动同步协程 —— 规格 §7.6 要求「没有手机在场时，同步照常进行」，而八个任务做完之后 `apply_sync` 的唯一调用点仍是那条要人点的 HTTP 路由。收集数因此从 1854 到 **1884**。

合并前的残留修复轮（1 条挡合并的 + 4 条收尾）又补了 4 条新测试：`apply_sync` 全部拷完之后、写账本之前盘才没的（1 条）、源目录还在但文件被删光不算拷成（1 条）、账本写不进去不许盖掉「备份盘满了」那句话（1 条）、自动同步那条循环扫盘连炸两次仍然活着（1 条）。另有一条既有测试被改准而不是新增：`test_拷到一半盘被拔掉_…` 原来断言的是账本被写到了那块已经不在的盘上，把 bug 固化成了期望。收集数因此从 1884 到 **1888**。


计划原本算的是 67 条（做完 1849），执行中长了 5 条，每一条都有由来：

- 第 6 个任务在计划给的 5 条之外补了 1 条 `verify_run` 的缺口测试——评审发现「目标盘上整个文件没拷过去」那条分支从没被走到过，原来 9 条全是改坏内容而不是删文件。
- 计划把第 8 个任务记成 8 条，漏算了追加到 `test_server_backup.py` 的那 1 条盘况断言。
- 第 8 个任务的评审又补了 3 条：一条真跑一遍 `run_preflight` 并断言结论里没有 backup 项（原来只扫签名，复用已有参数夹带备份校验不会被发现）、一条「不可用的镜像盘不算配了」的回归测试（夹具留了 `usable` 的口子却从没传过 `False`，而那正是最危险的静默故障）、一条 `plan_sync` 传不传 `runs=` 的等价性测试。
