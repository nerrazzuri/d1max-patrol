...
#: 占位符换成什么。一段普通的、不带斜杠的东西就够了。
#:
#: **它不带斜杠这件事是承重的**,所以底下那条 ``test_替换值本身不带斜杠``
#: 把它钉住:这里哪天被人改成一段带斜杠的东西,``CONTROLLED`` 里的 ``[^/]+``
#: 当场匹配不上,一堆本来受控的路由会被判成漏网(该绿判成红),而红的地方跟
#: 真正的原因隔着三层。(原来还有一个带斜杠的 ``_跨段``,给跨斜杠的告警键
#: ``<key*>`` 用;告警搬去站点(W00c5a)后跟着删了。)
_一段 = "x"
...
def test_没有跨斜杠的占位符(没起的服务):
    """占位符不跨斜杠,没有例外 —— 那是 ``_compile`` 那道穿越防线本身。

    原来有个跨斜杠的 ``<name*>``,只给告警键(``robot/kind#seq``)用;告警搬去
    站点(W00c5a,决策 8)后连同那个口子一起删了。现在正主不再认 ``<name*>``,
    写了会被当字面量 ``re.escape`` 掉 —— 那条路由静悄悄地永远匹配不上。所以这
    里钉死:注册过的路由里一个带星号的都不许有。真要重新开这个口子,先回答:
    这个参数会不会被拼进文件路径?会的话就别开。

    读的是运行时的 ``_routes``,不是去扫 ``server.py`` 的源文件(理由同
    :func:`枚举注册过的路由`)。
    """
    带星 = {模式 for _方法, 模式 in 枚举注册过的路由(没起的服务) if "*>" in 模式}
    assert not 带星, (
        f"{带星} 带 <name*>,而 server._compile 已经不认它了(会当字面量匹配)。")
    # 正主那一侧:一段带斜杠的值匹配不上任何占位符。
    assert not _compile("/api/runs/<run_id>/judge").match("/api/runs/a/b/judge")


def test_替换值本身不带斜杠():
    """:data:`_一段` 必须不带斜杠(见它上面那段注释)。"""
    assert "/" not in _一段, _一段
    # 顺手:CONTROLLED 里一条 GET 都没有(§3.5 规则 1)。
