...
class SiteScheduler:
    ...
    def _on_event(self, robot_id: str, e: Event) -> None:
        ...
        if result is None or not task_id:
            return
        with self.db.tx() as c:
            c.execute("UPDATE schedule_runs SET result=? WHERE task_id=? AND outcome='started'",
                      (result, task_id))
    ...
    async def tick(self) -> None:
        ...
        decisions = [(e, decide(e, now=now, last_started_ms=self.last_started(e.id)))
        ...
        for e, d in picked.displaced:
            self._record(e, d.scheduled_ms or 0, "displaced", note="同一拍有更优先的排程")
        if picked.chosen is not None:
            await self._start(act, *picked.chosen, now_ms=now_ms)

    def _candidates(self, act: ActiveBundle, entry: ScheduleEntry) -> tuple[list[str], str]:
    ...
    async def _start(self, act: ActiveBundle, entry: ScheduleEntry, d: Decision, *,
        ...
        except (DispatchRefused, DispatchTimeout) as exc:
            self._record(entry, scheduled_ms, "dispatch_failed", robot_id=rid, note=str(exc))
            return
        if r["ack"]["result"] != "accepted":
            self._record(entry, scheduled_ms, "dispatch_failed", robot_id=rid,
                         task_id=r["task_id"],
        ...
        self._record(entry, scheduled_ms, "started", robot_id=rid, task_id=r["task_id"],
                     note=d.kind.value)
